...
struct NewSprite: Instruction {
    ...
    func update(context: Script.ExecutionContext) -> Script.ExecutionContext {
        ...
        if let definitionName = context.arguments.string(for: NewSprite.definitionArgument),
           let aDefinition = spriteManager.pointee.definitions.first(where: { $0.nameAsString == definitionName }) {
            definition = aDefinition
        } else if let definitionIndex = context.arguments.integer(for: NewSprite.definitionArgument) {
    ...
    private func definitionIndex(sprite: MELSpriteRef, context: Script.ExecutionContext) -> Int {
        let definitionName = sprite.definition.nameAsString
        return context.spriteManager!.pointee.definitions.firstIndex(where: { $0.nameAsString == definitionName }) ?? 0
    }
}
